 ...
 */
 ...
 function files() {
  ...
  //example list -> Will be connected to a database
   let file_list = ["Nummer 1", "Nummer 2", "Nummer 3", "Nummer 4", "Nummer 5"]
   let file_elem = []
 
   //doesn't work FIX
   //-> innerHTML
   let head = document.getElementById("head")
   let style_file = document.createElement("style")
   style_file.setAttribute("href", "/styles/files.css")
   ...
   //doesn't work FIX
   page_content.appendChild(style_file)
 
   //center alignment doesn't work
   page_content.style.textAlign = "center";
   page_content.appendChild(text)
 
   file_list.forEach(element => {
     file_elem.push(`
       <a href="/files">
         <div class="li-elem" style="font-family:sans-serif;border: 1px black solid; margin: 1em; border-radius: .5em; padding: 2em; box-shadow: 5px 5px 10px 0px rgba(0,0,0,0.75);width:600px;margin-left:auto;margin-right:auto;background-color:#C9DDFF;">
           <h2 style="text-align: left; margin: .5em;">${element}</h2>
           <p>Das hier ist eine file-card<br/>In diesen werden später die Infos
           zu den jeweiligen Files stehen</p>
           <ol>
             <li style="text-align:left;"><a href="/files" style="color:blue;text-decoration:underline">Visualisierungen</a></li>
             <li style="text-align:left;"><a href="/files" style="color:blue;text-decoration:underline">Machine Learning Modelle</a></li>
           </ol>
         </div>
       </a>
       `)
   ...
   file_elem.forEach(element => {
     page_content.innerHTML += element
   });
   ...
   page_content.innerHTML += `
     <style>
       ...
       .add-btn:hover {
         box-shadow: 17px 17px 13px 0px rgba(0,0,0,0.75);
         right:.1em;
         bottom:.1em;
         cursor:pointer;
       }
 ...
 else if(window.location.pathname === "/files") {    //pathname would be something like "www.domain.com/documents" so the documents page
   files()
 }
